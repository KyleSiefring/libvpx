...
int vp9_optimize_b(MACROBLOCK *mb, int plane, int block, TX_SIZE tx_size,
  ...
  int16_t t0, t1;
  int i, final_eob;
  tran_low_t before_final_eob_qc, before_final_eob_dqc;
#if CONFIG_VP9_HIGHBITDEPTH
  ...
  final_eob = 0;
  before_final_eob_qc = 0;
  before_final_eob_dqc = 0;
  ...
  for (i = 0; i < eob; i++) {
    ...
    if (x == 0) {  // No need to search
      ...
      }
      {
        ...
        best_eob_cost_cur = eob_rdcost_better_for_x1 ? eob_cost1 : eob_cost0;

        // Determine whether to move the eob position to i+1
        if (best_eob_cost_cur < best_block_rd_cost) {
          best_block_rd_cost = best_eob_cost_cur;
          final_eob = i + 1;
          if (eob_rdcost_better_for_x1) {
            before_final_eob_qc = x1;
            before_final_eob_dqc = dqc1;
          } else {
            before_final_eob_qc = x;
            before_final_eob_dqc = dqc0;
          }
        }
      }
    }
  ...
  if (final_eob > 0) {
    int rc;
    assert(before_final_eob_qc != 0);
    i = final_eob - 1;
    rc = scan[i];
    qcoeff[rc] = before_final_eob_qc;
    dqcoeff[rc] = before_final_eob_dqc;
  }
