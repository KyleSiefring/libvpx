    ...
    for (column_start = 0; column_start < 32; column_start += 8) {
...
#if DCT_HIGH_BIT_DEPTH
            overflow = check_epi16_overflow_x2(&out_ptr[0], &out_ptr[8]);
            if (overflow) {
              HIGH_FDCT32x32_2D_ROWS_C(intermediate, output_org);
              return;
            }
...
#if DCT_HIGH_BIT_DEPTH
            overflow = check_epi16_overflow_x2(&out_ptr[4], &out_ptr[12]);
            if (overflow) {
              HIGH_FDCT32x32_2D_ROWS_C(intermediate, output_org);
              return;
            }
...
#if DCT_HIGH_BIT_DEPTH
            overflow = check_epi16_overflow_x4(&out_ptr[2], &out_ptr[10],
                                               &out_ptr[6], &out_ptr[14]);
            if (overflow) {
              HIGH_FDCT32x32_2D_ROWS_C(intermediate, output_org);
              return;
            }
...
#if DCT_HIGH_BIT_DEPTH
            overflow = check_epi16_overflow_x4(&out_ptr[1], &out_ptr[9],
                                               &out_ptr[7], &out_ptr[15]);
            if (overflow) {
              HIGH_FDCT32x32_2D_ROWS_C(intermediate, output_org);
              return;
            }
...
#if DCT_HIGH_BIT_DEPTH
            overflow = check_epi16_overflow_x4(&out_ptr[5], &out_ptr[13],
                                               &out_ptr[3], &out_ptr[11]);
            if (overflow) {
              HIGH_FDCT32x32_2D_ROWS_C(intermediate, output_org);
              return;
            }
      ...
      // Transpose the results, do it as four 4x16 transposes.
      {
        ...
        for (interleave = 0; interleave < 2; interleave++) {
          int16_t *output0 =
              &intermediate[(column_start + 4 * interleave) * 32];
          tran_low_t *output1 =
              &output_org[(column_start + 4 * interleave) * 32];
